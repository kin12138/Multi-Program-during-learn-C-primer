...
void Logger::flush() {
    // 等待队列清空
    while (!queue_.empty()) std::this_thread::sleep_for(std::chrono::milliseconds(10));
...
void Logger::workerThread() {
    ...
    while (running_ || !queue_.empty()) {
        LogMessage msg;
        if (queue_.wait_and_pop(msg)) {
            batch.push_back(std::move(msg));
        }

        // 如果队列为空，或批量已达阈值，则写盘
        if(batch.size() >= 32 || (!running_ && !batch.empty())) { // 消息累计达32 或在关闭阶段，只要还有未写入的消息
            writeBatch(batch);
            batch.clear();
        }
    }
